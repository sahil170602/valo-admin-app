...
function AdminDashboard({ onLogout }) {
    ...
    // UPDATED: Moment Modal State to handle Edit mode
...
useEffect(() => {
    ...
    // UPDATED: Save Moment (Handles Edit and Add)
    const handleSaveMoment = async (e) => {
        ...
        if (momentModal.mode === 'add') {
            ...
            await supabase.from('moments').update(newMoment).eq('id', momentModal.data.id);
        }
        
        setMomentModal({ open: false, mode: 'add', data: null });
    };

    // UPDATED: Added Confirmation for safety
        ...
        <div className="min-h-screen bg-slate-900 text-white font-sans flex overflow-hidden">
            ...
            <div className="flex-1 flex flex-col h-screen overflow-hidden">
                ...
                <main className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-900">
                    ...
                    {/* UPDATED MOMENTS GALLERY WITH EDIT & DELETE */}
                    {activeTab === 'moments' && ( 
    ...
    {/* EDIT ICON BUTTON */}
    <button 
        onClick={() => setMomentModal({ open: true, mode: 'edit', data: m })} 
        className="bg-blue-500 text-white p-2 rounded-md shadow-lg hover:bg-blue-400 transition flex items-center justify-center"
        title="Edit"
    >
    ...
    <button 
        ...
        className="bg-red-500 text-white p-2 rounded-md shadow-lg hover:bg-red-400 transition flex items-center justify-center"
        title="Delete"
    >
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4">
